...
edgeR_func <- function(species_to_analyze, study, unstim_name, stim_name, primate_full_name, rodent_full_name){
  ...
  study = 'Hagai'
  stim_name = c('LF4','PIC4')
  countsMatrix_path = "C:/Users/TzachiHNB6/Documents/lilach's paper/counts/counts_only_LF_PIC.txt"
  countsMatrix_path = paste("C:/Users/TzachiHNB6/Documents/lilach's paper/counts/",species_to_analyze,'_countData_',study,'.txt',sep='')
  countsMatrix<-read.table(countsMatrix_path,
                           header=T, sep = "\t",row.names=1)
  
  #reading the infotable
  info_table_path = paste("C:/Users/TzachiHNB6/Documents/lilach's paper/metadata/metadata_only_LF_PIC.txt")
  
  info_table_path = paste("C:/Users/TzachiHNB6/Documents/lilach's paper/metadata/small_metaData_",study,"_",species_to_analyze,".txt",sep='')
  info_table<-read.table(info_table_path,
                         header = T, sep='\t' ,row.names = 1)
  
  info_table_rel = info_table[info_table$treatment %in% stim_name,]
  ...
  # 
  # 
  #info_table$treatment = factor(info_table$treatment, levels = c("UNST", "IFNB", "PIC4", "LF4"))
  #info_table$species = factor(info_table$species, levels = c("Homo sapiens", "Mus musculus"))
  info_table$species <- factor(info_table$species, levels=c("Homo_sapiens","Mus_musculus"))
  info_table$treatment <- factor(info_table$treatment, levels=c("LF4", "PIC4"))
  countsMatrix = countsMatrix[,rownames(info_table_rel)]
  ...
  countsMatrix_rel_clean = countsMatrix_clean[,rownames(info_table_rel)]
  ...
  # 
  info_table_rel$treatment_and_species<-factor(paste(info_table_rel$treatment, info_table_rel$species, sep = "_"))
  
  
  # (C) create a DGE object
  #general
  edgeR_DE_obj <- DGEList(counts=countsMatrix_clean, group=info_table$group)
  ...
  # 
  info_table$treatment <- relevel(info_table$treatment, ref="LF4")
  
  design <- model.matrix(~0+species * treatment, data=info_table)
  colnames(design)
  edgeR_DE_obj <- estimateDisp(edgeR_DE_obj,design)
  fit <- glmFit(edgeR_DE_obj, design)
  # coef1 = (Human_PIC - Human.LF)
  # coef2 = (Mouse_PIC - Mouse.LF)
  # coef3 = (PIC vs LF)
  # coef4 =  (Mouse.PIC-Mouse.LF)-(Human.PIC-Human.LF)
  #contrast = c(0, 0, 0, 0, 0, 1, 1, 1)
  contrast <- makeContrasts(Mouse_PIC_vs_Human_LF = speciesMouse + treatmentPIC + `speciesMouse:treatmentPIC` , levels=design)
  
  lrt <- glmLRT(fit, coef = 4 )
  #lrt <- glmLRT(fit, contrast = c(-1,1,0,0) )
  topTags(lrt)
  lrt$table$qval<-p.adjust(lrt$table[,4], "BH") #fdr using the bh method
  
  
  f_stim= "C:/Users/TzachiHNB6/Documents/lilach's paper/results_MM_vs_HS.csv"
  ...
  print(f_stim)
  write.csv(lrt$table, file=f_stim)
...
edgeR_func(species_to_analyze = 'homo_vs_mus', study = 'Hagai',unstim_name = 'LF4', stim_name = c('LF4','PIC4'), primate_full_name = 'Homo sapiens', rodent_full_name = 'Mus musculus')
